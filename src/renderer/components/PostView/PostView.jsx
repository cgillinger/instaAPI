...
// Definiera specifika fält för per-inlägg-vyn - BEHÅLLER follows
...
const PostView = ({ data, selectedFields }) => {
  ...
  // Hämta unika konton från data - UPPDATERAD för account_name fallback
  useEffect(() => {
    if (data && Array.isArray(data)) {
      try {
        ...
        // Gå igenom varje inlägg och hämta kontonamn med fallback
  ...
  // Filtrera data baserat på valt konto - UPPDATERAD för account_name fallback
  const filteredData = React.useMemo(() => {
    if (!data || !Array.isArray(data)) return [];
    if (selectedAccount === ALL_ACCOUNTS) return data;
    
    return data.filter(post => {
      // KRITISK ÄNDRING: Jämför med både account_name och account_username
      const accountName = getValue(post, 'account_name') || getValue(post, 'account_username');
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const result = await window.electronAPI.exportToExcel(
        exportData,
        'instaapi-statistik-inlagg.xlsx'
      );
      if (result.success) {
  ...
  // Formatera data för export - UPPDATERAD för account_name fallback
  const formatDataForExport = (data) => {
    if (!data || !Array.isArray(data)) return [];
    
    return data.map(post => {
      // KRITISK ÄNDRING: Hantera account_name fallback
      const accountName = getValue(post, 'account_name') || getValue(post, 'account_username') || 'Ingen beskrivning';
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              >
                <div className="flex items-center">
                  {/* UPPDATERAD: Visa "Konto" istället för specifik fältnamn */}
                  Konto {getSortIcon('account_name')}
                </div>
              </TableHead>
          ...
          <TableBody>
            {paginatedData.map((post, index) => (
              <TableRow key={`post-${index}`}>
                ...
                {/* UPPDATERAD: Visa account_name med fallback till account_username */}
                <TableCell>
                  {formatValue(getValue(post, 'account_name') || getValue(post, 'account_username'))}
                </TableCell>
                {selectedFields.map(field => (
