...
// Definiera specifika fält för per-konto-vyn - BEHÅLLER follows
const ACCOUNT_VIEW_AVAILABLE_FIELDS = {
  ...
  'shares': 'Delningar',
  'saves': 'Sparade',
  'follows': 'Följare',  // BEHÅLLS: follows finns i nya CSV (0 för Reels)
...
// ProfileIcon-komponenten - UPPDATERAD för att hantera username
const ProfileIcon = ({ accountName }) => {
  // KRITISK ÄNDRING: Hantera att accountName kan vara ett användarnamn (t.ex. "p3poddar")
  ...
  // Använd kanalprefix om det finns en matchning, annars första bokstaven
...
// Funktion för att summera värden per konto (synkron version) - UPPDATERAD för account_name fallback
const summarizeByAccount = (data, selectedFields, columnMappings) => {
  ...
  for (const post of data) {
    ...
    // KRITISK ÄNDRING: account_name fallback till account_username
    const accountName = getValue(post, 'account_name') || getValue(post, 'account_username') || 'Okänt konto';
    const accountUsername = getValue(post, 'account_username') || getValue(post, 'account_name') || '-';
  ...
  for (const accountId in groupedByAccount) {
    ...
    for (const post of account.posts) {
      ...
      totalFollows += (getValue(post, 'follows') || 0);  // BEHÅLLS: follows (0 för Reels)
    }
    ...
    if (selectedFields.includes('follows')) summary.follows = totalFollows;  // BEHÅLLS
    ...
    if (selectedFields.includes('engagement_total_extended')) {
      // BEHÅLLS: Inkluderar follows (även om det är 0 för Reels)
...
const AccountView = ({ data, selectedFields }) => {
      ...
      if (Array.isArray(summary) && summary.length > 0) {
        ...
        if (Array.isArray(data)) {
          data.forEach(post => {
            ...
            totalShares += (getValue(post, 'shares') || 0);
            totalSaves += (getValue(post, 'saves') || 0);
            totalFollows += (getValue(post, 'follows') || 0);  // BEHÅLLS: follows
          });
        }
        ...
        if (selectedFields.includes('follows')) totals.follows = totalFollows;  // BEHÅLLS
  ...
  // Hantera klick på extern länk - UPPDATERAD för account_username
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const result = await window.electronAPI.exportToExcel(
        exportData,
        'instaapi-statistik-konton.xlsx'
      );
      if (result.success) {
  ...
  // Formatera data för export - UPPDATERAD för account_username
  const formatDataForExport = (data) => {
    return data.map(account => {
      // KRITISK ÄNDRING: Använd account_name eller account_username
      const displayName = getValue(account, 'account_name') || getValue(account, 'account_username') || 'Unknown';
      
      const formattedAccount = {
        'Kontonamn': displayName
      };
      
      // Lägg till Instagram-URL om användarnamn finns
      const username = getValue(account, 'account_username') || getValue(account, 'account_name');
              ...
              <TableRow key={`${getValue(account, 'account_id')}-${getValue(account, 'account_name')}`}>
                ...
                <TableCell className="font-medium">
                  <div className="flex items-center space-x-2">
                    {/* UPPDATERAD: Skicka account_name eller account_username till ProfileIcon */}
                    <ProfileIcon accountName={getValue(account, 'account_name') || getValue(account, 'account_username')} />
                    <span>{getValue(account, 'account_name') || getValue(account, 'account_username') || 'Unknown'}</span>
                  </div>
                </TableCell>
                {selectedFields.map((field) => (
                ...
                <TableCell className="text-center">
                  {/* UPPDATERAD: Använd account_username för länk */}
