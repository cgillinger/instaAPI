...
// Displaynamn för tillgängliga fält i per-konto vyn - BEHÅLLER follows
...
// Displaynamn för tillgängliga fält i per-inlägg vyn - BEHÅLLER follows
...
export const getUniquePageNames = (data) => {
  ...
  data.forEach(post => {
    // KRITISK ÄNDRING: Försök hämta kontonamn från account_name först, sedan account_username
    const accountName = post.account_name || post.account_username || post['Account name'] || post['Account'];
    if (accountName) {
 ...
 * UPPDATERAD: Behåller follows och hanterar account_name fallback
 */
  ...
  const groupedByAccount = data.reduce((acc, post) => {
    ...
    if (!acc[accountId]) {
      ...
      acc[accountId] = {
        ...
        account_name: post.account_name || post.account_username || 'Okänt konto',
        account_username: post.account_username || post.account_name || '-',
        posts: []
      };
    }
  ...
  const summaryData = Object.values(groupedByAccount).map(account => {
    ...
    selectedFields.forEach(field => {
      if (field === 'average_reach') {
        ...
        // BEHÅLLS: Beräkna grundläggande engagemang (likes + comments + shares)
        summary.engagement_total = account.posts.reduce((sum, post) => {
        ...
        // BEHÅLLS: Beräkna utökat engagemang (inklusive follows och saves)
        summary.engagement_total_extended = account.posts.reduce((sum, post) => {
          const likes = post.likes || 0;
          const comments = post.comments || 0;
          const shares = post.shares || 0;
          const saves = post.saves || 0;
          const follows = post.follows || 0;  // BEHÅLLS: follows (0 för Reels)
          return sum + likes + comments + shares + saves + follows;
        }, 0);
      } else {
        // Summera övriga värden - INKLUSIVE follows
