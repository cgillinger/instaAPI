...
// Summeringsbara värden för "Per konto"-vy - BEHÅLLER follows (finns i nya CSV)
const SUMMARIZABLE_COLUMNS = Object.values(DEFAULT_MAPPINGS).filter(col => [
  "views", "likes", "comments", "shares", "saves", "follows"  // BEHÅLLS: follows finns i nya CSV
].includes(col));

// Metadata och icke-summeringsbara värden
const NON_SUMMARIZABLE_COLUMNS = Object.values(DEFAULT_MAPPINGS).filter(col => [
  "post_id", "account_id", "account_username", "description",  // ÄNDRAT: account_username
 ...
 * UPPDATERAD: Hantera både account_id och account_username
 */
function countUniqueAccounts(data) {
  ...
  data.forEach(row => {
    // Försök med account_id först, sedan account_username som fallback
    const accountId = getValue(row, 'account_id') || getValue(row, 'account_username');
    if (accountId) {
 ...
 * UPPDATERAD: Hantera account_username istället för account_name
 */
      ...
      Papa.parse(csvContent, {
        ...
        complete: (results) => {
          ...
          filteredData.forEach(row => {
            ...
            // KRITISK ÄNDRING: Använd account_username för att få accountID
            const accountID = getValue(mappedRow, 'account_id') || getValue(mappedRow, 'account_username') || 'unknown';
            
            if (!accountID) return;
            
            // KRITISK ÄNDRING: Hantera att account_name kan vara samma som account_username
            const accountUsername = getValue(mappedRow, 'account_username') || getValue(mappedRow, 'account_name') || 'Okänt konto';
            const accountName = getValue(mappedRow, 'account_name') || accountUsername; // Fallback till username om name saknas
            ...
            if (!perKonto[accountID]) {
              perKonto[accountID] = { 
                ...
                "account_name": accountName,               // Kan vara samma som username
                "account_username": accountUsername        // Primär identifierare i nya CSV
              };
          ...
          Object.values(perKonto).forEach(account => {
            ...
            account.engagement_total_extended =
              ...
              (account.shares || 0) +
              (account.saves || 0) +
              (account.follows || 0);  // BEHÅLLS: follows finns i nya CSV
          });
 ...
 * UPPDATERAD: Hantera account_username fallback
 */
export function getUniquePageNames(data) {
  ...
  data.forEach(post => {
    // Försök account_name först, sedan account_username som fallback
    const accountName = getValue(post, 'account_name') || getValue(post, 'account_username');
    if (accountName) {
